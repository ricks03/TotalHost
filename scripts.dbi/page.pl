...
&validate($cgi,$session) unless ($in{'client'} eq 'java');
...
if ($in{'client'} eq 'java') {
    ...
    print "</body></html>\n";
    &LogOut(100,"client: $in{'client'}, GameFile: $in{'GameFile'}",$LogFile);
    exit;
}
...
if ($in{'lp'} eq 'profile') { 
  ...
  if ($user_admin eq $userlogin) { $menu_left{"6Admin"} = "$WWW_Scripts/page.pl?lp=profile&cp=show_admin"; } # Display the admin panel to the Admin
...
if ($in{'cp'} eq 'edit_profile') {  
  ...
  if ($GameValues{'NewsPaper'}) { $in{'rp'} = 'show_news'; }# if news was enabled, it needs to display
	...
	print "</td>";
} elsif ($in{'cp'} eq 'Upload Zip') { 
	print "<td>"; 
  # If the Game File isn't set, set it at random
  ...
  &display_warning;
	...
	} else { print "No active games found for your User ID.\n"; $in{'rp'} = 'games'; }
	print "</td>";
...
# And the CANCEL button uses the value of "show_games"
	...
	#my $sql = qq|SELECT Games.GameFile, Games.GameName, Games.GameStatus, Games.GameDescrip, Games.HostName, Games.NewsPaper, Games.GameType, Games.HourlyTime, Games.AsAvailable, Games.Teams FROM Games ORDER BY Games.GameStatus, Games.NextTurn DESC;|;
  my $sql = qq|SELECT Games.GameFile, Games.GameName, Games.GameStatus, Games.GameDescrip, Games.HostName, Games.NewsPaper, Games.GameType, Games.HourlyTime, Games.AsAvailable, Games.Teams FROM Games ORDER BY CASE WHEN Games.GameStatus = 7 THEN 0 ELSE 1 END, Games.GameStatus, Games.NextTurn DESC;|;
	print "<td>"; &list_games($sql, 'All Games'); &print_legend(4, 0, %StatusBall); print "</td>";
} elsif ($in{'cp'} eq 'show_my_games_inprogress') {
	my $sql = qq|SELECT Games.GameFile, Games.GameName, Games.GameStatus, Games.GameDescrip, Games.HostName, Games.NewsPaper, Games.GameType, Games.HourlyTime, Games.AsAvailable, Games.Teams FROM Games INNER JOIN GameUsers ON (Games.GameFile = GameUsers.GameFile) AND (Games.GameFile = GameUsers.GameFile) WHERE GameUsers.User_Login=? AND Games.GameStatus>1 AND Games.GameStatus<6 ORDER BY Games.GameStatus, Games.NextTurn;|; #my $sql = qq|SELECT Games.GameFile, Games.GameName, Games.GameStatus, Games.GameDescrip, Games.HostName, Games.NewsPaper, Games.GameType, Games.HourlyTime, Games.AsAvailable, Games.Teams FROM Games INNER JOIN GameUsers ON (Games.GameFile = GameUsers.GameFile) AND (Games.GameFile = GameUsers.GameFile) WHERE GameUsers.User_Login='$userlogin' AND Games.GameStatus>1 AND Games.GameStatus<6 ORDER BY Games.GameStatus, Games.NextTurn;|;
	print "<td>"; &list_games($sql, 'My Games In Progress', $userlogin); print "</td>";
} elsif ($in{'cp'} eq 'show_games_inprogress') {
	my $sql = qq|SELECT Games.GameFile, Games.GameName, Games.GameStatus, Games.GameDescrip, Games.HostName, Games.NewsPaper, Games.GameType, Games.HourlyTime, Games.AsAvailable, Games.DayFreq, Games.HourFreq, Games.DailyTime, Games.Teams FROM Games WHERE Games.GameStatus>1 AND Games.GameStatus<6 ORDER BY Games.GameStatus, Games.NextTurn;|;
	print "<td>"; &list_games($sql, 'Games In Progress'); print "</td>";
} elsif ($in{'cp'} eq 'show_games_completed') {
	my $sql = qq|SELECT Games.GameFile, Games.GameName, Games.GameStatus, Games.GameDescrip, Games.HostName, Games.NewsPaper, Games.GameType, Games.HourlyTime, Games.AsAvailable, Games.DayFreq, Games.HourFreq, Games.DailyTime, Games.Teams FROM Games WHERE Games.GameStatus=9 ORDER BY Games.GameStatus;|;
  ...
  &show_race($First{'RaceID'},$session->param("userlogin")); 
	...
	print "<td>"; &display_warning($in{'status'}); print "</td>";
...
## Teams
  ...
  if ($GameValues{'NewsPaper'}) { $in{'rp'} = 'show_news'; }
    ...
    if ($GameValues{'NewsPaper'}) { $in{'rp'} = 'show_news'; }
		...
		print "<td>"; 
    &process_email($in{'GameFile'}, $in{'Message'}, $in{'GameName'},$in{'EmailType'}); 
    &display_warning;
    &show_game($in{'GameFile'}); print "</td>";
    ...
    # Redirect back to the Add Delay panel
    $in{'cp'} = 'Add Delay';
    &show_game($in{'GameFile'});
    print "</td>";
    ...
    # Don't need to rebuild the .chk 
    # Redirect back to the Replace Player panel for ease for zip uploads
  ...
  &show_admin;
  print "</td>";
} elsif ($in{'cp'} eq 'process_admin') { # display the admin portal on the profile page
	print "<td>";
  &process_admin;
  print "</td>";
} else {	
...
if ($in{'rp'} eq 'my_games') { 
  ...
  #my $sql = qq|SELECT Games.GameFile, Games.GameName, Games.GameStatus, Games.NewsPaper FROM User INNER JOIN (Games INNER JOIN GameUsers ON Games.GameFile = GameUsers.GameFile) ON User.User_Login = GameUsers.User_Login GROUP BY Games.GameFile, Games.GameName, Games.GameStatus, Games.NewsPaper, User.User_ID HAVING User.User_ID=| . $session->param("userid") . qq| ORDER BY Games.GameStatus, Games.GameName;|;
  #my $sql = qq|SELECT Games.GameFile, Games.GameName, Games.GameStatus, Games.NewsPaper FROM User INNER JOIN (Games INNER JOIN GameUsers ON Games.GameFile = GameUsers.GameFile) ON User.User_Login = GameUsers.User_Login GROUP BY Games.GameFile, Games.GameName, Games.GameStatus, Games.NewsPaper, User.User_ID HAVING User.User_ID=| . $session->param("userid") . qq| UNION SELECT Games.GameFile, Games.GameName, Games.GameStatus, Games.NewsPaper FROM Games WHERE Games.HostName = '$userlogin' ORDER BY GameStatus, GameName;|;
  #my $sql = qq|SELECT Games.GameFile, Games.GameName, Games.GameStatus, Games.NewsPaper FROM User INNER JOIN (Games INNER JOIN GameUsers ON Games.GameFile = GameUsers.GameFile) ON User.User_Login = GameUsers.User_Login GROUP BY Games.GameFile, Games.GameName, Games.GameStatus, Games.NewsPaper, User.User_ID HAVING User.User_ID=? UNION SELECT Games.GameFile, Games.GameName, Games.GameStatus, Games.NewsPaper FROM Games WHERE Games.HostName = ? ORDER BY GameStatus, GameName;|; 
  my $sql = qq|SELECT Games.GameFile, Games.GameName, Games.GameStatus, Games.NewsPaper FROM User INNER JOIN (Games INNER JOIN GameUsers ON Games.GameFile = GameUsers.GameFile) ON User.User_Login = GameUsers.User_Login GROUP BY Games.GameFile, Games.GameName, Games.GameStatus, Games.NewsPaper, User.User_ID HAVING User.User_ID=? UNION SELECT Games.GameFile, Games.GameName, Games.GameStatus, Games.NewsPaper FROM Games WHERE Games.HostName = ? ORDER BY CASE WHEN GameStatus = 7 THEN 0 ELSE 1 END, GameStatus, GameName;|;
	&rp_list_games($sql, 'My Games', 'my_games', $session->param("userid"), $userlogin); #&rp_list_games($sql, 'My Games', 'my_games');
	print "</td>\n";
...
#	my $sql = "SELECT GameName, GameFile, GameStatus FROM Games WHERE GameStatus>1 AND GameStatus<6;";
  my $sql = "SELECT GameName, GameFile, GameStatus, NewsPaper FROM Games WHERE GameStatus>1 AND GameStatus<6;";
	&rp_list_games($sql,'Games In Progress', 'games');
	print "</td>\n";
# Display a list of new games
  ...
  my $sql = "SELECT GameName, GameFile, GameStatus, NewsPaper FROM Games WHERE GameStatus = 7 OR GameStatus = 0;";
	&rp_list_games($sql,'New Games', 'games_new');
	print "</td>\n";
# Display a list of games needing a replacement player
  ...
  my $sql = "SELECT GameName, GameFile, GameStatus, NewsPaper FROM Games WHERE GameStatus = 5;";
	&rp_list_games($sql,'Games needing Players', 'games_replacement');
	print "</td>\n";
# Display a list of completed games
  ...
  my $sql = "SELECT GameName, GameFile, GameStatus, NewsPaper FROM Games WHERE GameStatus = 9 ORDER BY Games.NextTurn DESC;"; 
	&rp_list_games($sql,'Completed Games', 'games_complete');
	print "</td>\n";
...
print qq|</td></tr>\n|;
print qq|</table>\n|;
print qq|<INPUT type="Checkbox" name="EmailTurn" value=$EmailTurn | . &button_help("EmailTurns") . qq|$Checked[$EmailTurn]>Receive Turn Notifications via Email</P>\n|;
print qq|<INPUT type="Checkbox" name="EmailList" value=$EmailList | . &button_help("EmailNewGames") . qq|$Checked[$EmailList]>Receive New Game Notifications</P>\n|;
print qq|<input type=submit name="Submit" value="Update">\n|;
...
sub show_admin {
	# Display the admin portal - admin only
	# Check authorization
	if ($userlogin ne $user_admin) {
		print qq|<P><b style="color: red;">Access Denied: You are not authorized to access the admin portal.</b></P>\n|;
		&LogOut(0, "Unauthorized access attempt to show_admin by $userlogin", $ErrorLog);
		return;
	}
	...
	my $sql = qq|SELECT User_ID, User_Login, User_First, User_Last, User_Email FROM User ORDER BY User_Login;|;
	my @users = ();
	...
	print qq|<H2>Admin</H2>\n|;
	print qq|<P>Users cannot be deleted if they are in any game</P>\n|;
	
	print <<eof;
<FORM name="delete_user" method="$FormMethod" action="$WWW_Scripts/page.pl">
<input type="hidden" name="lp" value="profile">
<input type="hidden" name="cp" value="process_admin">
<input type="hidden" name="rp" value="">
<table>
<tr><td>Delete User:</td>
<td>
...
eof
	...
	foreach my $user (@users) {
		print qq|<option value="$user->{'User_ID'}">$user->{'User_Login'} - $user->{'User_First'} $user->{'User_Last'} ($user->{'User_Email'})</option>\n|;
	}
...
<td>
<BUTTON $admin_style type="submit" name="delete_user" value="Delete User">Delete User</BUTTON>
</td>
</tr>
</table>
</FORM>
...
sub process_admin {
	# Process admin actions - admin only
	# Check authorization
	if ($userlogin ne $user_admin) {
		print qq|<P><b style="color: red;">Access Denied: You are not authorized to perform admin actions.</b></P>\n|;
		&LogOut(0, "Unauthorized access attempt to process_admin by $userlogin", $ErrorLog);
		return;
	}
	
	my $db = &DB_Open($dsn);
	
	if ($in{'delete_user'} && $in{'selected_user_id'}) {
		my $target_user_id = $in{'selected_user_id'};
		
		# First check if user exists in any games
		my $check_sql = qq|SELECT Games.GameName, Games.GameFile FROM GameUsers 
		                   INNER JOIN Games ON GameUsers.GameFile = Games.GameFile 
		                   INNER JOIN User ON GameUsers.User_Login = User.User_Login 
		                   WHERE User.User_ID = ?;|;
		
		my @user_games = ();
		if (my $sth = &DB_Call($db, $check_sql, $target_user_id)) {
			while (my $row = $sth->fetchrow_hashref()) {
				push @user_games, "$row->{'GameName'} ($row->{'GameFile'})";
			}
			$sth->finish();
		}
		
		if (@user_games) {
			# User is in games, display warning
			print qq|<P><b style="color: red;">Cannot delete user - User is currently in the following games:</b></P>\n|;
			print qq|<ul>\n|;
			foreach my $game (@user_games) {
				print qq|<li>$game</li>\n|;
			}
			print qq|</ul>\n|;
			print qq|<P>Please remove the user from these games before deleting the account.</P>\n|;
			&LogOut(100, "Admin: Attempted to delete user ID $target_user_id but user is in games", $LogFile);
		} else {
			# Get user details before deletion
			my $user_sql = qq|SELECT User_Login, User_File FROM User WHERE User_ID = ?;|;
			my ($user_login, $user_file);
			if (my $sth = &DB_Call($db, $user_sql, $target_user_id)) {
				my $row = $sth->fetchrow_hashref();
			...
			if ($user_login) {
				...
				my $delete_sql = qq|DELETE FROM User WHERE User_ID = ?;|;
				if (my $sth = &DB_Call($db, $delete_sql, $target_user_id)) {
					print qq|<P><b style="color: green;">User $user_login (ID: $target_user_id) deleted from database.</b></P>\n|;
					&LogOut(100, "Admin: User $user_login (ID: $target_user_id) deleted by $userlogin", $LogFile);
					$sth->finish();
					
					# Delete user folder if it exists
					if ($user_file) {
						my $user_dir = $Dir_Races . '/' . $user_file;
						use File::Path 'rmtree';
						if (-d $user_dir && $user_file && (length($user_file) > 0)) {
							rmtree(&clean($user_dir));
							print qq|<P>User folder deleted: $user_file</P>\n|;
							&LogOut(100, "Admin: User folder $user_file deleted by $userlogin", $LogFile);
						} elsif (!(-d $user_dir)) {
							print qq|<P>User folder did not exist: $user_file</P>\n|;
							&LogOut(100, "Admin: User folder $user_file did not exist", $LogFile);
						}
					}
				} else {
					print qq|<P><b style="color: red;">Failed to delete user from database.</b></P>\n|;
					&LogOut(0, "Admin: Failed to delete user ID $target_user_id", $ErrorLog);
				}
			} else {
				print qq|<P><b style="color: red;">User not found.</b></P>\n|;
				&LogOut(0, "Admin: User ID $target_user_id not found", $ErrorLog);
			}
		}
	}
	&DB_Close($db);
	# Redisplay the admin form
	&show_admin;
}
...
sub process_game_remove {
  ...
  my $sql = qq|DELETE GameUsers FROM GameUsers JOIN Games ON Games.GameFile = GameUsers.GameFile WHERE Games.HostName = ?  AND GameUsers.GameFile = ? AND GameUsers.PlayerID = ?;|;
 	my $db = &DB_Open($dsn);
	if (my $sth = &DB_Call($db,$sql,$userlogin,$GameFile,$PlayerID)) { 
    &LogOut(100,"process_game_remove: $userlogin removed $PlayerID from game $GameFile $sql" , $LogFile);
    $sth->finish(); 
  }
	&DB_Close($db);
}
...
sub show_game {  
  ...
  # Determine if this is a team game with merged files by checking the folder for files ending in .merged
  my $gamedir = $Dir_Games . '/' . $GameFile; 
  #my $teamgame = &has_merged($gamedir);
    ...
    my $timezone = $GameValues{'User_Timezone'} || 'America/New_York'; # In case we aren't logged in to get that info
    ...
    print "<tr>\n";
    if ($GameValues{'GameStatus'} == 7) { print "<td>Game Status: New Game - Pending new players</td>\n";
		} elsif ($GameValues{'GameStatus'} == 0) { print "<td>Game Status: New Game - Locked, Waiting for Host to Start</td>\n";
    } else { 
    ...
    print qq|<tr><td>Host ID: <a href="mailto:$GameValues{'User_Email'}">$GameValues{'HostName'}</a></td>\n|;
    
    # Change the Button style if the current user is not the host, but is admin
     if ( $GameValues{'HostName'} ne $user_admin && $session->param("userlogin") eq $user_admin) {
       $host_style = $admin_style;
     }
    
    # Display the Replace Host options and button if applicable                                       
    if ($in{'cp'} eq 'Replace Host' && ($session->param("userlogin") eq $GameValues{'HostName'} || $session->param("userlogin") eq $user_admin)) {
      print qq|<td>|;
      print qq|<FORM method="$FormMethod" action="$WWW_Scripts/page.pl" name="my_form">\n<SELECT name=\"ReplaceHost\">\n|;
      ...
      # Sort and limit to active players
    	...
    	print qq|</SELECT>|;
      print qq|<BUTTON $host_style type="submit" name="Update Host" value="Update Host" | . &button_help('ReplaceHost') .  qq|>Update Host</BUTTON>\n|;
      print qq|<input type="hidden" name="Update Host" value="Update Host">\n|;
      print qq|<input type="hidden" name="lp" value="profile_game">\n|;
      print qq|<input type="hidden" name="rp" value="my_games">\n|;
      print qq|<input type="hidden" name="cp" value="Update Host">\n|;
    ...
    if (-f $GameDefFile) {
      ...
      my $dtlast = DateTime->from_epoch(epoch => $mtime, time_zone => $timezone);
    ...
    if (($GameValues{'NextTurn'} > 0) && ($GameValues{'GameType'} == 1 || $GameValues{'GameType'} == 2) ) { 
          ...
          my $dtnext = DateTime->from_epoch(epoch => $next_turn_epoch, time_zone => $timezone);
          my $dtduration = $dtnext->epoch - time();
          my $dthours = int(abs($dtduration) / 3600);
          my $dtminutes = int((abs($dtduration) % 3600) / 60);
          if ($dtduration > 0) {
            print "<br><b>Next turn due on or before: " . $dtnext->strftime('%a %Y-%m-%d %H:%M:%S %Z') . " ($dthours hours, $dtminutes minutes)</b>\n";
          } else { print "<br><b>Turn Generation IMMINENT!</b>\n"; }
...
#         print "<br>Last turn generation: " . strftime("%Y-%m-%d %H:%M:%S %Z", localtime($GameValues{'LastTurn'})) . "\n";
      my $dtlast = DateTime->from_epoch(epoch => $GameValues{'LastTurn'}, time_zone => $timezone);
      print "<br>Last turn generation: " . $dtlast->strftime('%a %Y-%m-%d %H:%M:%S %Z') . "\n";       
      }	else { print "<br>No turns have been generated yet.\n"; }
    } 
    
    my $dtnow = DateTime->from_epoch(epoch => time(), time_zone => $timezone);
    print "<br>Now: " . $dtnow->strftime('%a %Y-%m-%d %H:%M:%S %Z');
    
		# If next turn is undefined(0) AND it's a game in progress somehow, display that the 
		# next generation will be immediate
		if ($GameValues{'GameStatus'} != 7 && $GameValues{'GameStatus'} != 9 && $GameValues{'GameStatus'} != 4) { 
      if ($GameValues{'GameType'} == 3) { print "<P>Manual generation only"; }
      else { print "<P>Will gen with/on the next automated generation"; }
...
#       print $turngen;
      ...
      print &show_schedule($GameValues{'GameType'}, $GameValues{'HourlyTime'}, $GameValues{'AsAvailable'}, $GameValues{'DayFreq'}, $GameValues{'HourFreq'}, $GameValues{'DailyTime'},0);
      print "<P>\n";
      # Display Active Game data if an active game and this data exists
		  # display the game and player information from the .chk File
      
  		print "<table>\n";
      if ($GameValues{'Teams'}) { print "<tr><th></th><th></th><th></th><th></th><th>Merged</th><th></th><th></th><th></th></tr>\n"; }
      ...
      # Display player status, one line for each player in the .chk file
  		while (defined $CHK[$Position]) {  #Write .m file lines
  			my ($CHK_Status, $CHK_Name, $CHK_Id) = &Eval_CHKLine($CHK[$Position]); # Return the status and race name    
        ...
        if ($CHK_Status =~ /Error/) { print qq|<tr><td colspan="5">CHK File error: $CHK_Status</td></tr>|; $Position++; $Player++; next; }
          ...
          my $MFileMerged = $Dir_Games . '/' . $GameFile . '/' . $GameFile . '.m' . $Player . '.merged';
          ...
          $sql = qq|SELECT Games.GameFile, User.User_File, GameUsers.User_Login, GameUsers.PlayerID, GameUsers.PlayerStatus, GameUsers.DelaysLeft, GameUsers.Team, _PlayerStatus.PlayerStatus_txt FROM _PlayerStatus INNER JOIN (User INNER JOIN (Games INNER JOIN GameUsers ON (Games.GameFile = GameUsers.GameFile)) ON User.User_Login = GameUsers.User_Login) ON _PlayerStatus.PlayerStatus = GameUsers.PlayerStatus WHERE (((Games.GameFile)=?) AND ((GameUsers.PlayerID)=?));|;  			
        ...
        # These are TH values, not Stars! values
  			if ($PlayerValues{'PlayerStatus'} == 1) { $del = ''; $del2 = '';  # Active
...
#   			else { print qq|<td><img src="$TurnBall{$CHK_Status}" alt='Status' border="0" name="$CHK_Status"></a></td>\n|; }
        my $ball_key;
        if    ($PlayerValues{'PlayerStatus'} == 2 || $PlayerValues{'PlayerID'} eq '') { $ball_key = 'AI';         }
        elsif ($CHK_Status eq 'Out' && $PlayerValues{'PlayerStatus'} == 4)            { $ball_key = 'Idle';       }
        elsif ($CHK_Status eq 'Out' && $PlayerValues{'PlayerStatus'} == 3)            { $ball_key = 'Banned';     }
        else                                                                          { $ball_key = $CHK_Status; }
        print qq|<td><img src="$TurnBall{$ball_key}" alt='Status' border="0" name="$CHK_Status"></a></td>\n|;  
        ...
        # Display the .m file link(s)
#         #print "<P>MFile: $MFile, Merged: $MFileMerged, Teams: $GameValues{'Teams'}, login: $PlayerValues{'User_Login'}, $userlogin \n";
#         if (-f $MFile) { # Only show if the file is present)
#           if ($CHK_Status eq 'Deceased' || $PlayerValues{'PlayerStatus'} == 3 || $PlayerValues{'PlayerStatus'} == 2) {  # no link if the player is dead, banned, being edited, or AI
#             print "<td>.m$Player</td>\n";
#           } elsif ($GameValues{'SharedM'} ) { # Always display link if .m files are shared
#     	 			print qq|<td><A href=\"$WWW_Scripts/download.pl?file=$GameFile.m$Player\">.m$Player</A></td>\n|;
#             if ( $GameValues{'Teams'} ) { # There should only be merged files if this is a team game
#               if (-f $MFileMerged ) { print qq|<td align="center"><a href=\"$WWW_Scripts/download.pl?file=$GameFile.m$Player.merged\">.m$Player</A></td>\n|; } else { print qq|<td align="center" title="Merged file will be available after the next turn is generated">(pending)</td>\n|; }
#             } 
#     	 		} elsif ($PlayerValues{'User_Login'} eq $userlogin  ) { # Display link if the logged in user is the player
#             print qq|<td><A href=\"$WWW_Scripts/download.pl?file=$GameFile.m$Player\">.m$Player</A></td>\n|; 
#             if ( $GameValues{'Teams'} ) { # There should only be merged files if this is a team game
#               if (-f $MFileMerged ) { print qq|<td align="center"><a href=\"$WWW_Scripts/download.pl?file=$GameFile.m$Player.merged\">.m$Player</A></td>\n|; } else { print qq|<td align="center" title="Merged file will be available after the next turn is generated">(pending)</td>\n|; }
#             } 
#           } elsif (!$playeringame && $GameValues{'HostName'} eq $userlogin && $GameValues{'HostAccess'}) { # Display link if player is host but not in game
#             print qq|<td><A href=\"$WWW_Scripts/download.pl?file=$GameFile.m$Player\">.m$Player</A> (Host Access)</td>\n|; 
#             if ( $GameValues{'Teams'} ) { # There should only be merged files if this is a team game
#               if (-f $MFileMerged ) {print qq|<td align="center"><a href=\"$WWW_Scripts/download.pl?file=$GameFile.m$Player.merged\">.m$Player</A> (Host Access)</td>\n|; } else { print qq|<td align="center" title="Merged file will be available after the next turn is generated">(pending)</td>\n|; }
#             } 
#     			} else { 
#             print qq|<td>.m$Player</td>\n|; 
#             if ( $GameValues{'Teams'} ) { # There should only be merged files if this is a team game
#               if (-f $MFileMerged ) { print qq|<td align="center">.m$Player</td>|; } else { print qq|<td align="center" title="Merged file will be available after the next turn is generated">(pending)</td>\n|; }
#             } 
#           }
        if (-f $MFile) { # Only show .m link if the file is present
          my $mlink;
          my $mmergedlink;
          # No link if the player is dead, or player is Inactive or Idle 
          if ($CHK_Status eq 'Deceased' || $PlayerValues{'PlayerStatus'} == 2 || $PlayerValues{'PlayerStatus'} == 3) {
            $mlink = ".m$Player";
            $mmergedlink = ".m$Player";
          # if sharedM is set, the player is the logged-in player, or Host Access is set and the host is not in the game, or admin 
          } elsif ($GameValues{'SharedM'} || $PlayerValues{'User_Login'} eq $userlogin || (!$playeringame && $GameValues{'HostName'} eq $userlogin && $GameValues{'HostAccess'}) || $userlogin eq $user_admin) {
            my $suffix = (!$playeringame && $GameValues{'HostName'} eq $userlogin && $GameValues{'HostAccess'}) ? ' (Host Access)' : '';
            $suffix .= ' (Admin)' if ($userlogin eq $user_admin && $PlayerValues{'User_Login'} ne $userlogin);
            $mlink      = qq|<A href="$WWW_Scripts/download.pl?file=$GameFile.m$Player">.m$Player</A>$suffix|;
            $mmergedlink = qq|<a href="$WWW_Scripts/download.pl?file=$GameFile.m$Player.merged">.m$Player</a>$suffix|;
          } else { # For all other cases, no link
            $mlink = ".m$Player";
            $mmergedlink = ".m$Player";
          }
          print qq|<td>$mlink</td>\n|;
          
          if ($GameValues{'Teams'}) { # If a Team game show the merged .m file
        ...
        if ($in{'cp'} eq 'Remove PWD' && ($session->param("userlogin") eq $GameValues{'HostName'} || $session->param("userlogin") eq $user_admin) && $PlayerValues{'PlayerID'} ne '') {
          print qq|<td align=center style="padding:0;margin:0;">|;
          print qq|<FORM method="$FormMethod" action="$WWW_Scripts/page.pl" name="my_form" style="padding:0;margin:0;">\n|;
        ...
        if ($in{'cp'} eq 'Replace Player' && ($session->param("userlogin") eq $GameValues{'HostName'} || $session->param("userlogin") eq $user_admin) && $PlayerValues{'PlayerID'} ne '') {
      		# Get the User List
	        print qq|<td valign="middle" style="padding:0;margin:0;"><FORM method="$FormMethod" action="$WWW_Scripts/page.pl" name="my_form" style="padding:0;margin:0;">\n<SELECT name=\"ReplaceName\">\n|;
          # Sort and limit to active players
        	$sql = "SELECT * FROM User WHERE User_Status=1 ORDER BY User_Login;";
        	if (my $sth = &DB_Call($db,$sql)) {
            while (my $row = $sth->fetchrow_hashref()) {
              my $selected = ($row->{'User_Login'} eq $PlayerValues{'User_Login'}) ? ' SELECTED' : '';
              print qq|<OPTION value="$row->{'User_Login'}"$selected>$row->{'User_Login'}</OPTION>\n|;
        		}
            $sth->finish();
        	}
        	print qq|</SELECT>|;
        ...
        if ($in{'cp'} eq 'Add Delay' && ($session->param("userlogin") eq $GameValues{'HostName'} || $session->param("userlogin") eq $user_admin) && $PlayerValues{'PlayerID'} ne '') {
	        print qq|<td valign="bottom" style="padding:0;margin:0;"><FORM method="$FormMethod" action="$WWW_Scripts/page.pl" name="my_form" style="padding:0;margin:0;">\n|;
        ...
        if (($in{'cp'} eq 'Change Status') && ($userlogin eq $PlayerValues{'User_Login'})) {
	        print qq|<td valign="bottom"><FORM method="$FormMethod" action="$WWW_Scripts/page.pl" name="my_form" style="padding:0;margin:0;">\n|;
          # Display current avalable delays
      		if (($PlayerValues{'PlayerStatus'} == 1) && ($GameValues{'GameStatus'} != 9)) { 
          ...
          if (($PlayerValues{'PlayerStatus'} == 4) && ($GameValues{'GameStatus'} != 9)) { 
        ...
        # Store the current player ID for future reference
  		...
  		if (($GameValues{'GameStatus'} =~ /^[234]$/) && ($current_player eq $userlogin)) {
  	  		&show_upload($GameValues{'GameName'},$GameFile,0);
  		} 
    } else {
    ...
    # Display Refresh Button. Fixed to not require a warning or resubmit
    ...
    #if ($GameValues{'GameStatus'} =~ /^[23479]$/) { print qq|<A href="$WWW_Scripts/page.pl?lp=game&cp=Refresh&rp=$in{'rp'}&GameFile=$GameValues{'GameFile'}"><BUTTON $user_style type="button" | . &button_help('Refresh') . qq|>Refresh</BUTTON></A>\n|; $button_count = &button_check($button_count);}
    if ($GameValues{'GameStatus'} =~ /^[23479]$/) { print qq|<BUTTON $user_style type="button" onclick="window.location.href='$WWW_Scripts/page.pl?lp=game&cp=Refresh&rp=$in{'rp'}&GameFile=$GameValues{'GameFile'}'" | . &button_help('Refresh') . qq|>Refresh</BUTTON>\n|; $button_count = &button_check($button_count);}
    
    ###if ($GameValues{'GameStatus'} =~ /^[23479]$/) { print qq|<A href="$WWW_Scripts/page.pl?lp=game&cp=Refresh&rp=$in{'rp'}&GameFile=$GameValues{'GameFile'}" style="display:inline-block;width:120px;height:24px;text-align:center;line-height:24px;text-decoration:none;" | . &button_help('Refresh') . qq|>Refresh</A>\n|; $button_count = &button_check($button_count);}
    ####if ($GameValues{'GameStatus'} =~ /^[23479]$/) { print qq|<FORM method="GET" action="$WWW_Scripts/page.pl" style="display:inline;"><INPUT type="hidden" name="lp" value="game"><INPUT type="hidden" name="cp" value="Refresh"><INPUT type="hidden" name="rp" value="$in{'rp'}"><INPUT type="hidden" name="GameFile" value="$GameValues{'GameFile'}"><BUTTON $user_style type="submit" | . &button_help('Refresh') . qq|>Refresh</BUTTON></FORM>\n|; $button_count = &button_check($button_count);}
    ####if ($GameValues{'GameStatus'} =~ /^[23479]$/) { print qq|</FORM><FORM method="GET" action="$WWW_Scripts/page.pl" style="display:inline;margin:0;padding:0;"><INPUT type="hidden" name="lp" value="game"><INPUT type="hidden" name="cp" value="Refresh"><INPUT type="hidden" name="rp" value="$in{'rp'}"><INPUT type="hidden" name="GameFile" value="$GameValues{'GameFile'}"><BUTTON $user_style type="submit" | . &button_help('Refresh') . qq|>Refresh</BUTTON></FORM><FORM action="$WWW_Scripts/page.pl" method="$FormMethod">\n<input type="hidden" name="lp" value="profile_game">\n<input type="hidden" name="rp" value="my_games">\n<input type="hidden" name="GameFile" value="$GameValues{'GameFile'}">\n<input type="hidden" name="GameName" value="$GameValues{'GameName'}">\n<input type="hidden" name="HostName" value="$GameValues{'HostName'}">\n<input type="hidden" name="User_File" value="$player_file">\n|; $button_count = &button_check($button_count);}
    ...
    # Lock the game and prepare to start
		if (($GameValues{'HostName'} eq $userlogin || $admin ) && $GameValues{'GameStatus'} eq '7' && $playercount >= 2) { print qq|<BUTTON $host_style type="submit" name="cp" value="Lock Game" | . &button_help('LockGame') . qq|>Lock Game</BUTTON>\n|; $button_count = &button_check($button_count);}
		# Unlock the game 
    ...
    # Submit a news article by player or host
		...
		# Change the team status
		if (($GameValues{'HostName'} eq $session->param("userlogin") || $admin )  && ($GameValues{'GameStatus'} =~ /^[234]$/) && $GameValues{'Teams'} == 1) 		{ print qq|<BUTTON $host_style type="submit" name="cp" value="Team Status" | . &button_help('TeamStatus') . qq|>Team Status</BUTTON>\n|; $button_count = &button_check($button_count);}
    ...
    # Email Hosts
		if ($admin && $players) { print qq|<BUTTON $host_style type="submit" name="cp" value="Email Hosts" | . &button_help('EmailHosts') . qq|>Email Hosts</BUTTON>\n|; $button_count = &button_check($button_count);}
    # Email All
		if ($admin && $players) { print qq|<BUTTON $host_style type="submit" name="cp" value="Email All" | . &button_help('EmailAll') . qq|>Email All</BUTTON>\n|; $button_count = &button_check($button_count);}
    ...
    # And I don't have host id when I get there because I'm matching on user to be more secure.
		...
		#Movies  # Don't provide button if there's already a movie. 
    ...
    if ($GameValues{'GameStatus'} =~ /^[2345]$/ && $GameValues{'Exploit'} && $fixFiles) {
      &show_fix($GameFile); 
    }
    ...
    &print_legend(5,1,%TurnBall);

		&read_game($GameFile); # Display the TH game parameters
		&read_def($GameFile); # Display the Stars game parameters
...
sub show_client {  
  ...
  my $dtnow = DateTime->now(time_zone => 'UTC');  # Create a DateTime object with the current time in UTC
    ...
    my $tz = $timezone || 'America/New_York'; # In case we aren't logged in to get that info
    ...
    if ($GameValues{'NextTurn'} != 0 && $GameValues{'GameStatus'} != 7 && $GameValues{'GameStatus'} != 9 && $GameValues{'GameStatus'} != 4) { 
			if ($GameValues{'GameStatus'} == 4) { print "<br>next-gen=[PAUSED]\n"; }
      elsif ($GameValues{'GameType'} == 3) { print "<br>next-gen-time=Manual generation only"; }
      else { 
        #print "<br>next-gen-time=" . strftime("%Y-%m-%d %H:%M:%S %Z", localtime($GameValues{'NextTurn'})) . "\n"; 
        my $dtnext = DateTime->from_epoch(epoch => $GameValues{'NextTurn'}, time_zone => $tz);
        print "<br>next-gen-time=" . $dtnext->strftime('%Y-%m-%d %H:%M:%S %Z') . "\n"; 
      }
    }
    ...
    my $mtime = $stats[9];
    my $dtcreated = DateTime->from_epoch(epoch => $mtime, time_zone => $tz);
    print "<br>game-created=" . $dtcreated->strftime('%Y-%m-%d %H:%M:%S %Z') . "\n"; 
       
    # Get the time for "now"
    my $dtcurrent = DateTime->from_epoch(epoch => time(), time_zone => $tz);
    print "<br>current-time=" . $dtcurrent->strftime('%Y-%m-%d %H:%M:%S %Z') . "\n";  
    ...
    # As available    
    print "<br>generate-as-available=$GameValues{'AsAvailable'}\n"; 
   ...
   # Display the player information
    ...
    if ($GameValues{'GameStatus'} != 7 && $GameValues{'GameStatus'} != 6 && $GameValues{'GameStatus'} != 0) { 
      ...
      # Display player status, one line for each player in the .chk file
  		while (defined $CHK[$Position]) {  #Write .m file lines
  			my ($CHK_Status, $CHK_Name, $CHK_Id) = &Eval_CHKLine($CHK[$Position]);    
        # If an error is reported in the CHK file (like Host File Locked) report it and then move on.
...
#			while ( my ($key, $value) = each(%GameUserValues) ) { print "<br>$key => $value\n"; }
  ...
  if ($GameValues{'HostName'} ne $userlogin && $userlogin ne $user_admin) {
    ...
    #chdir("/home/www-data/.wine/drive_c") or die "Cannot change directory: $!";
		...
		sleep 3; # Give Stars! time to create all the files
      ...
      # Generate merged files if this is a team game
      if ($GameValues{'Teams'}) { &MergeFiles($GameFile); }   
      ...
      # set the game status to paused and send email 
			&process_game_status($GameFile, $session->param("userlogin"), 'Launched', $userlogin); 
			&DB_Close($db);
			return 1;
		} else {
	...
	# otherwise delete it
      ...
      $sth->finish();
		...
		if ($RaceValues{'RaceID'} && $RaceValues{'User_Login'}) {
			$sql= "DELETE FROM Races WHERE RaceID = ? AND User_Login = ?;"; #$sql= "DELETE FROM Races WHERE (RaceID=$RaceValues{'RaceID'} AND User_Login=\'$RaceValues{'User_Login'}\');";
			&LogOut(200,"delete_race: $sql",$SQLLog);
			if (my $sth = &DB_Call($db,$sql,$RaceValues{'RaceID'},$RaceValues{'User_Login'})) { #if (my $sth = &DB_Call($db,$sql)) { 
...
sub edit_game {
	my ($type) = @_;
	print "<td>";
  if ($in{'status'}) { &display_warning($in{'status'}); } 
	$db = &DB_Open($dsn);
	if ($type eq 'edit') {
...
#	elsif ( $GameValues{'GameType'} == 4) { $allin = 1; }
  ...
  my $key_minutes;
    ...
    } elsif ($key == $GameValues{'HourlyTime'} && $key >= 1 ) { 
      if ($key == 1) { print qq|<OPTION value=$key SELECTED>$key hour\n|; }
      else { print qq|<OPTION value=$key SELECTED>$key hours\n|; } 
  	} elsif ($type eq 'create' && $key == 48) { 
      if ($key == 1) { print qq|<OPTION value=$key SELECTED>$key hour\n|; }
      else { print qq|<OPTION value=$key SELECTED>$key hours\n|; } 
  	} else { 
      # Display minutes or hours as appropriate
      if ($key > 1) { print qq|<OPTION value=$key>$key hours\n|;
      } elsif ($key == 1) { print qq|<OPTION value=$key>$key hour\n|;
      } elsif ($key < 1) { 
        $key_minutes = int(($key * 60) +.5);
	...
	print qq|<INPUT name="GameType" type="radio" value=3 | . &button_help("AsRequired") . qq| $Checked[$required]>As Required (Manual)<BR>\n|;
...
eof
  ...
  print qq|<TD><INPUT type="checkbox" name="PublicMessages" | . &button_help("PublicMessages") . qq| $Checked[$GameValues{'PublicMessages'}]>Public Messages</TD>\n|;
	print qq|</TR><TR>\n|;
  print qq|<TD><INPUT type="checkbox" name="Teams" | . &button_help("Teams") . qq| $Checked[$GameValues{'Teams'}]>Teams</TD>\n|;
	print qq|</TR></TABLE>\n|;
...
sub delete_game {
  ...
  # Confirm that the user logged in is the host or admin, and that the game returns from the sql query 
  # matches the request
  if (($userlogin eq $GameValues{'HostName'} || $userlogin eq $user_admin) && ($GameValues{'GameFile'} eq $GameFile) && $GameFile) {
    ...
    # (Shouldn't be necessary, as the relationship should take them out
    ...
    print qq|<P>Game user database entries deleted for: $GameValues{'GameName'} by $userlogin.\n|;
  	my $sth = &DB_Close($db);
    # Delete the game files, carefully using the database values, not the user input.
    ...
    if(-d $dir && $GameValues{'GameFile'} && (length($GameValues{'GameFile'}) > 0)) { 
      rmtree(&clean($dir));
      print "<P>Game files deleted for: $GameValues{'GameName'} by $userlogin.";
    } else { 
    ...
    my $graph_file = $Dir_Graphs . '/graphs/' . $GameValues{'GameFile'} . '.png';
    if (-f $graph_file) {
        unlink($graph_file);
        print "<P>Graph file deleted for: $GameValues{'GameName'} by $userlogin.";
    } else {
        print "<P>Graph file for $GameValues{'GameFile'} does not exist.";
        &LogOut(0,"Delete of Graph file $graph_file by $userlogin failed as it does not exist.",$ErrorLog);
    }
    # Delete the movie file
    my $movie_file = $Dir_Graphs . '/movies/' . $GameValues{'GameFile'} . '.gif';
    if (-f $movie_file) {
        unlink($movie_file);
        print "<P>Movie file deleted for: $GameValues{'GameName'} by $userlogin.";
    } else {
        print "<P>Movie file for $GameValues{'GameFile'} does not exist.";
        &LogOut(0,"Delete of movie file $movie_file by $userlogin failed as it does not exist.",$ErrorLog);
    }    
...
sub update_game {
  ...
  if (!$in{'GameType'})   { $in{'GameType'} = 3; }       # Default to Daily if not set
  ...
  if  (($ForceGenTurns * $ForceGenTimes ) > 500) {
    ...
    if ($ForceGenTimes > 50) { $ForceGenTimes = 50; }
  }
  ...
  my $Teams = &checkboxnull($in{'Teams'}); 
  ...
  } else { &updateList($in{'GameFile'}, 1); }  
    
  my $Sanitize = &checkboxnull($in{'Sanitize'});
  ...
  if (!($PublicMessages)) { 
    ...
    if ($Teams) { push @Values, "Team Game"; } 
    ...
    if (my $sth = &DB_Call($db,$sql,@bind_params)) { 
       ...
       &LogOut(0,"$in{'GameName'} update $sql failed for $userlogin", $ErrorLog); 
		...
		&LogOut(50,"Creating random GameFile $GameFile for $in{'GameName'}",$LogFile);
		  ...
		  my @bind_params = (
  			...
  			$AsAvailable, $OnlyIfAvailable, $DayFreq, $HourFreq,
  			$ForceGen, $ForceGenTurns, $ForceGenTimes,
  			$HostMod, $HostForceGen, $NoDuplicates, $GameRestore,
  			$AnonPlayer, $GamePause, $GameDelay, $NumDelay, $MinDelay,
     ...
     $sth->finish(); 
      ...
      $Subject = $mail_prefix . 'Game Creation: ' . $in{'GameFile'};
      $Message = "\n\nA game has been created: $in{'GameName'} ($in{'GameFile'})\n";
      $smtp = &Mail_Open;
...
sub read_def {
  ...
  # Tiny: 400x400, Small: 800x800, Medium: 1,200x1,200, Large: 1,600x1,600, Huge: 2,000x2,000
  ...
  if (!-f $def_file) {
    if (-f $xy_file) {
      ...
      # export a new  .df2 (full with player info) file
      open(my $fh, '>', "$df2_file") or &LogOut(0, "Cannot open file: $!", $ErrorLog);
      foreach my $value (@GameValues) { print $fh "$value\n"; }
      close($fh);
      
      # export a new  .def file (without player info)
      open(my $fh, '>', "$def_file") or &LogOut(0, "Cannot open file: $!", $ErrorLog);
      my $count = $GameValues[3];        # 4th element, number of players
      splice(@GameValues, 4, $count);   # remove the player entries, as they're only in the df2 file.
      $GameValues[3] = 0; # Set player count to 0, because that's the default in the std def file.
      foreach my $value (@GameValues) { print $fh "$value\n"; }
      close($fh);
    }
  }
	if (-f $def_file) { #Check to see if file is there.
	  ...
	  #	if ($univ_random) { push(@Vals, "Game Seed: $univ_random"); }
		...
		print "<P>Game Definition File not found for $GameFile!\n"; 
...
sub read_game {
    ...
    my $hourly_result;
     if ($GameValues{'HourlyTime'} < 1) { $hourly_result =  int(($GameValues{'HourlyTime'} * 60)) . " minutes"; } # int for 10 minute 1.67
      ...
      if ($hourly_result == 1) { $hourly_result =  $GameValues{'HourlyTime'} . ' hour'; }
      else { $hourly_result =  $GameValues{'HourlyTime'} . ' hours';} 
    }
    push(@Values, "Hourly Turn Gen, $hourly_result");
  }
	elsif ( $GameValues{'GameType'} == 3) { push(@Values, "Turns as Required (Manual)"); }
#	elsif ( $GameValues{'GameType'} == 4) { push(@Values, "Turns Gen Only when All Turns are In"); }
	...
	if ($GameValues{'Teams'}) { push (@Values, "Team Game"); }
  ...
  if ($GameValues{'Exploit'})  { push (@Values, "Exploit Detection"); }
  if ($GameValues{'Sanitize'}) { push (@Values, "File Sanitization"); }
	my $c = 0;
	my $col = 4;
...
sub show_restore {
  ...
  print qq|<h2>Restore Game for: $GameValues{'GameName'} ($GameValues{'GameFile'})</h2>\n|;
	...
	print qq|</SELECT></td>\n|;
 	...
 	print qq|<td><INPUT type=\"hidden\" name=\"HostName\" value =\"$HostName\"></td>\n|;
	print qq|</tr><tr>\n|;
...
#			while ( my ($key, $value) = each(%GameValues) ) { print "<br>$key => $value\n"; }
      # We need to print a bunch of this only once if the same player is in the same game more than once
        ...
        print "<H2>Submit a delay for: $GameValues{'GameName'} ($GameValues{'GameFile'})</H2>\n";
        print &show_schedule($GameValues{'GameType'}, $GameValues{'HourlyTime'}, $GameValues{'AsAvailable'}, $GameValues{'DayFreq'}, $GameValues{'HourFreq'}, $GameValues{'DailyTime'},0);
...
sub process_delay {
	...
	my ($GameFile, $HostName, $delay_turns, $PlayerID) = @_;
...
sub show_upload { # Uses $GameName and $GameFile
	my($GameName,$GameFile, $UploadType) = @_;   # Zip=1  or not.
  ...
  if ($UploadType) {
    print qq|<td><INPUT type="submit" name="submit" value="Upload File" | . &button_help('SendZip') . qq|></td>\n|;
  } else {
    print qq|<td><INPUT type="submit" name="submit" value="Upload Turn" | . &button_help('SendTurn') . qq|></td>\n|;  
  }
	print qq|</tr>\n</table>\n|;
	# send GameName
...
#			while ( my ($key, $value) = each(%GameValues) ) { print "<br>$key => $value\n"; }
	...
	print "<table>\n";
 	...
 	while ($LoopPosition <= ($#PlayerData)) { # work the way through the array
		...
		print qq|<input type="hidden" name="GameFile" value="$GameFile">\n|;
		print qq|<input type="hidden" name="HostName" value="$HostName">\n|;
		print qq|<input type="hidden" name="PlayerStatus" value=$PlayerData[$LoopPosition]{'PlayerStatus'}>\n|;
    my $PlayerID = $LoopPosition+1;
		print qq|<input type="hidden" name="PlayerID" value= $PlayerData[$LoopPosition]{'PlayerID'}>\n|;
		#print "<td>User ID:</td>\n";
  ...
  print qq|<P>Idle simply ignores turn submission status for turn generation.|;
...
sub process_player_status {
  ...
  my ($GameFile, $HostName, $UserFile, $NewPlayerStatus, $PlayerStatus, $PlayerID) = @_; # Player ID 1-16
  ...
  if ($GameUserStatus{'PlayerStatus'} == 3 && $GameStatus{'HostName'} ne $userlogin && $userlogin ne $user_admin) { # Only a host can un ban someone
    $update = 0;
    &LogOut(100, "process_player_status: Failed attempt to unbanStatus updated to $NewPlayerStatus for $GameFile by $userlogin",$ErrorLog);
  }
  ...
  if ($update) {
    ...
    # Don't need for Idle as that's a player status thing
    if ($NewPlayerStatus eq 'Inactive' || $NewPlayerStatus eq 'Active') {
    ...
    &LogOut(400, "process_player_status: updateStatus: $updateStatus for $GameFile by $userlogin",$LogFile);
    
    if ($updateStatus) {
      &LogOut(400, "process_player_status: Update approved: $GameFile, $HostName, $UserFile, $NewPlayerStatus, $PlayerStatus, $PlayerID", $LogFile);
      #  Update the database
...
# Show current player team status
sub show_team_status {
	my ($GameFile, $HostName) = @_;
	my %PlayerValues;
	my @PlayerData;
  my @Status;
  my @TXT;
	# Read in all the player data
  my $sql = qq|SELECT Games.GameFile, Games.GameName, Games.NewsPaper, User.User_Login, User.User_File, Games.HostName, Games.AnonPlayer, GameUsers.PlayerID, GameUsers.DelaysLeft, GameUsers.PlayerStatus, GameUsers.Team, _PlayerStatus.PlayerStatus_txt FROM User INNER JOIN (_PlayerStatus INNER JOIN (Games INNER JOIN GameUsers ON (Games.GameFile = GameUsers.GameFile)) ON _PlayerStatus.PlayerStatus = GameUsers.PlayerStatus) ON User.User_Login = GameUsers.User_Login WHERE (((Games.GameFile)=?)) ORDER BY GameUsers.PlayerID;|;
	my $db = &DB_Open($dsn);
	if (my $sth = &DB_Call($db,$sql,$GameFile)) { 
		while (my $row = $sth->fetchrow_hashref()) { 
      %PlayerValues = %{$row};  
#			while ( my ($key, $value) = each(%GameValues) ) { print "<br>$key => $value\n"; }
	...
	print "<H2>Update Teams for: $PlayerData[0]{'GameName'}</H2>\n";
	print "<table>\n";
 	my $LoopPosition = 0; #Start with the first player in the array.
 	while ($LoopPosition <= ($#PlayerData)) { # work the way through the array
		print "<tr>\n";
    ...
    print qq|<td>Team: <SELECT name="NewTeamStatus">\n|;
    my $currentTeam = $PlayerData[$LoopPosition]{'Team'} || 0;
    # None option
    if ($currentTeam == 0) { print qq|<OPTION value="0" SELECTED>None\n|;
    } else { print qq|<OPTION value="0">None\n|; }

    # Team Options 1-8
    for (my $i = 1; $i <= 8; $i++) {
        if ($currentTeam == $i) { print qq|<OPTION value="$i" SELECTED>$i\n|; 
        } else { print qq|<OPTION value="$i">$i\n|;  }
    }
    print qq|</SELECT></td>\n|;
		print qq|</td>\n|;
		print qq|<td><INPUT type="submit" name="cp" value="Update Team" onMouseOver="Help( \'UpdateTeam\' )" onMouseOut="Help( \'blank\' )">\n|; 
		print qq|</td>\n|;
		print qq|</FORM>\n|;
		print qq|</tr>\n|;
		$LoopPosition++;
	...
	print "</table>\n";
  print "<P>Members of a team will get full access to the planet, fleet, design, object, and score data built into their .m file\n";
  print " for the most recent turn.\n";
}                                            

sub process_team_update {
  # Process the results of a host changing a team status. 
  my ($GameFile, $HostName, $NewTeamStatus, $PlayerID) = @_; # Player ID 1-16
  ...
  if (my $sth = &DB_Call($db,$sql,$NewTeamStatus,$GameFile,$PlayerID)) {
    $sth->finish();
    # email players                                                                           
    # First, get the name and email address of all the players for this game. 
    $sql = qq|SELECT Games.GameFile, Games.GameName, User.User_Login, User.User_Email, GameUsers.PlayerID, GameUsers.Team FROM User INNER JOIN (Games INNER JOIN GameUsers ON Games.GameFile = GameUsers.GameFile) ON User.User_Login = GameUsers.User_Login WHERE Games.GameFile = ?;|;    
    if (my $sth = &DB_Call($db,$sql,$GameFile)) { 
      #			while ( my ($key, $value) = each(%GameValues) ) { print "<br>$key => $value\n"; }
      while (my $row = $sth->fetchrow_hashref()) { 
        %PlayerValues = %{$row};   
      ...
      # Not displaying the player name solves several problems, not the least is 
      # not having that value, and revealing anonymous players
      $Subject = "$mail_prefix $PlayerData[0]{'GameName'} : Team Status Change";
      $Message = "Player $PlayerID team changed to $NewTeamStatus in $PlayerData[0]{'GameName'}.";
      $smtp = &Mail_Open;
      while ($LoopPosition <= ($#PlayerData)) { # work the way through the array
        $MailTo = $PlayerData[$LoopPosition]{'User_Email'};
        &Mail_Send($smtp, $MailTo, $MailFrom, $Subject, $Message);
      ...
      &Mail_Close($smtp); 
    } else { &LogOut(10,"process_team_update: team  status failed updating PlayerID: $PlayerID for User $User_Login in $GameFile",$ErrorLog);}
  }
  &DB_Close($db);
}
...
# Display the interface to force generate turns. 
  ...
  if ($userlogin ne $HostName && $userlogin ne $user_admin) {
    &LogOut(0,"submit_forcegen: Unauthorized attempt. Hostname: $HostName, GameFile: $GameFile, User Login: $userlogin",$ErrorLog); 
    return;
  }
...
sub process_forcegen {
  # Process turn force generation
	...
	# Only the game host can force gen or admin
	if (($HostName eq $userlogin && $GameValues{'HostForce'}) || $userlogin eq $user_admin) {
		&GenerateTurn($NumberofTurns, $GameFile); 
    &cleanFiles($GameFile, $GameValues{'Sanitize'}); # Clean files if that's enabled
    &updateList($GameFile, $GameValues{'Exploit'});  # update List files for exploit detection
    if ($GameValues{'Teams'}) { &MergeFiles($GameFile); }  # Merge .m files for teams
...
sub process_remove_password {
  my ($GameFile, $PlayerID, $HostName) = @_;
  require StarsBlock;  
  require File::Copy;
...
sub show_email {
# Display the interface to send email to all active game players/hosts/ players.
	my ($GameFile, $GameName, $Type) = @_;

  print "<td>\n";
  if ($Type eq 'Email Players')    { print "<H2>Send Email to all players in: $GameName</H2>"; }
  elsif ($Type eq 'Email Hosts')   { print "<H2>Send Email to all hosts of active games</H2>"; }
  elsif ($Type eq 'Email All')     { print "<H2>Send Email to all active players in active games</H2>"; }
...
# This routine sends email 
	my ($GameFile, $Message, $GameName, $Type) = @_; 
  my ($sql, $dbh, $smtp); 
  ...
  $GameValues{'GameName'} = $GameName; # Package it up for later
  ...
  } elsif ($Type eq 'Email Hosts' || $Type eq 'Email All' ) {
    &LogOut(400, "process_email: $userlogin sending email for $Type: $Message", $LogFile); 
    $Subject = $mail_prefix;
    if ($Type eq 'Email Hosts') { 
      $Subject .= "Email to all Hosts";
      $sql = "SELECT DISTINCT User.User_Email FROM User JOIN Games ON User.User_Login = Games.HostName WHERE User.User_Status = 1  AND Games.GameStatus IN (2, 3, 4, 5);"; 
    }
    ...
    foreach my $row (@results) {
      ...
      $MailFrom = $mail_from;
      # Loop through all responses from the SQL Query
      &Mail_Send($smtp, $MailTo, $MailFrom, $Subject, $Message);
    }    
    &Mail_Close($smtp);
    &LogOut(100, "Email sent to $Type for $Subject, $Message", $LogFile); 
   }
	print qq|Email sent!\n|;
}
...
# see sub process_player_status {
  ...
  &LogOut(400, qq|switch_player: GameFile: $GameFile Player ID: $PlayerID, ReplaceName: $ReplaceName, Login: $userlogin|, $LogFile);
  ...
  if ($GameValues{'HostName'} eq $userlogin || $userlogin eq $user_admin) {
    ...
    # Log the events
   	&LogOut(200,qq|switch_player: Gamefile: $GameFile $userlogin swapped Player $PlayerID to $ReplaceName|, $LogFile);
  } else { &LogOut(50, qq|switch_player: $GameFile $userlogin attempted to swap Player $PlayerID to $ReplaceName|, $ErrorLog);}  
  &DB_Close($db);
}

sub process_switch_host {
  my ($GameFile, $Host, $ReplaceName) = @_;
  my %GameValues;
  
  &LogOut(400, qq|switch_host: GameFile: $GameFile Host $Host, ReplaceName: $ReplaceName, Login: $userlogin|, $LogFile);
  ...
  if ($GameValues{'HostName'} eq $session->param("userlogin") || $session->param("userlogin") eq $user_admin) {
   $sql = qq|UPDATE Games SET HostName = ? WHERE HostName = ? AND GameFile = ?;|;
    ...
    # Log the events
   	&LogOut(200,qq|switch_host: GameFile: $GameFile $userlogin swapped Host $Host to $ReplaceName|, $LogFile);
  } else { &LogOut(50, qq|switch_host: GameFile: $GameFile $userlogin attempted to swap Host $Host to $ReplaceName|, $ErrorLog);}  
  &DB_Close($db);
}
