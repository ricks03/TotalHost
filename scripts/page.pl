...
sub list_races {
	...
	if (&DB_Call($db,$sql)) {
		while ($db->FetchRow()) {
	    ($RaceName, $RaceFile, $RaceID) = $db->Data("RaceName", "RaceFile", "RaceID");
			$c++;
...
eof
			...
			print "<P>$sql . No Races found. Would you like to <a href=\"/scripts/page.pl?lp=profile_race&cp=upload_race&rp=my_races\">upload one</a>?\n";
...
sub process_join_game {
  ...
  if ($playercount > $GameValues{'MaxPlayers'}) { 
    ...
    &LogOut(0, "$userlogin Failed to join game $GameFile because the game was at MaxPlayers $GameValues{'MaxPlayers'}", $LogFile);
      ...
      my %RaceValues;
      if (&DB_Call($db,$sql)) { 
        $db->FetchRow(); %RaceValues = $db->DataHash(); 
      }
  		my $now = time();
