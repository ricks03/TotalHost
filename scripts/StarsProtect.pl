...
if (!($inDir)) { 
  ...
  print "\nUsage: \n\tStarsProtect.pl <File Directory> \n\n";
  print "Example: \n";
  print "\tStarsProtect.pl c:\\games \n\n";
  print "If there's more than one game in the same folder, you can \n";
  print "specify the game on the command line:\n";
  print "\tStarsProtect.pl c:\\games c:\\games\\Game.x1 \n\n";
...
while (defined(my $file = readdir(DIR))) {  
    ...
    my @block9data = &readBlock9($filename);
...
# Loop through the .x files
foreach my $file1 ( sort keys %block9 ) {
  print "Checking: $file1\n";
  # Check it against all the elements in the array
  foreach my $file2 ( sort keys %block9 ) {
    if ($file1 eq $file2) { next; } # if it's the same element then skip it
    ...
    if (@{$block9{$file1}}[0] eq @{$block9{$file2}}[0]) {
      ...
      if (@{$block9{$file1}}[1] eq @{$block9{$file2}}[1]) {
        print "\tInfo   : Same serial/hardware hash as $file2\n";
      } else { 
        print "\tDANGER : Same serial as $file2, but different hardware hash\n"; 
      }
    } else { 
      print "\tOK     : Different serial than $file2\n"; 
    } 
  } 
  print "\n";
} 
